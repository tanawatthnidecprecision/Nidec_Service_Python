...
class FileUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            token += str(random.randint(0, 10))
            token += str(random.randint(0, 10))
            hashGenerate = hashlib.sha256(
                ("FILES"+token).encode('utf-8')).hexdigest().upper()
            filename = fs.save(hashGenerate + '.' + uploaded_file.name.split('.')
                               [len(uploaded_file.name.split('.'))-1], uploaded_file)
            file_url = fs.url(filename)
            print('flie upload:', filename, file_url)
