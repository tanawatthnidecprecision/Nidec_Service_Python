...
class Department(APIView):
    ...
    def post(self, request):
        value = ""
        # handleEvent.handleToken(request) and
        if (request.data['department_index']):
            value = DatabaseQuery.selectWhere(
                'department_control', '*', 'department_index = \'{}\''.format(request.data['department_index']))[0]
    ...
    def post(self, request):
        ...
        if (handleEvent.handleToken(request) and request.POST['department_index']):
            value = Database.selectWhere(
                'position_control', '*', 'position_index = \'{}\''.format(request.POST['position_index']))[0]
        return JsonResponse({'status': 'successful', 'data': value} if value != "" else {'status': 'error'}, safe=False)
...
class InfoUserList(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            value = DatabaseQuery.selectWhere(
                'user_list', '*', 'user_index = \'{}\''.format(val))[0]
            #value = (value.slice(0,1)+value.slice(1)).replaceAll('"','').split(",")
        return JsonResponse({'status': 'successful', 'data': value} if value != "" else {'status': 'error'}, safe=False)
...
class PictureList(APIView):
    ...
    def post(self, request):
        ...
        if (handleEvent.handleToken(request) and request.get['user_index']):
            print('picture_list', '*',
                  'user_index = \'{}\''.format(request.POST['user_index']))
            value = Database.selectWhere(
                'picture_list', '*', 'user_index = \'{}\''.format(request.POST['user_index']))[0]
        return JsonResponse({'status': 'successful', 'data': value} if value != "" else {'status': 'error'}, safe=False)
    ...
    def get(self, request):
        ...
        if (request.session.get('login')):
            value = Database.selectWhere(
                'picture_list', '*', 'user_index = \'{}\''.format(request.session.get('login')[0]))[0][0]
        return JsonResponse({'status': 'successful', 'data': value} if value != "" else {'status': 'error'}, safe=False)
...
class NotifyIntranets(APIView):
    ...
    def post(self, request):
        value = Database.selectWhere(
            'approve_alarm', '*', 'user_index = 7150', 'slip')[0]
        return JsonResponse({'status': 'successful', 'data': value} if value != "" else {'status': 'error'}, safe=False)
